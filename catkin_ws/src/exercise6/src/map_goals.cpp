...
void pointCallback(geometry_msgs::Vector3 point) {
    int x = point.x;
    int y = point.y;
    ...
    geometry_msgs::Vector3 transformed;

    //goal.header.frame_id = "map";
    //goal.pose.orientation.w = 1;
    transformed.x = transformed_pt.x;
    transformed.y = transformed_pt.y;
    transformed.z = 0.0;
    //goal.header.stamp = ros::Time::now();

    ROS_INFO("Transformed to: (x: %f, y: %f)", transformed_pt.x, transformed_pt.y);

    point_pub.publish(transformed);
}
...
int main(int argc, char** argv) {
    ...
    point_sub = n.subscribe("infl_map_point", 10, &pointCallback);
    point_pub = n.advertise<geometry_msgs::Vector3>("transformed_map_point", 10);
