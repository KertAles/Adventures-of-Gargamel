...
cloud_cb (const pcl::PCLPointCloud2ConstPtr& cloud_blob)
{
  ...
  int min_inlier_points = 40;
  int min_hull_size = 25;
 ...
 // Extract the planar inliers from the input cloud
  ...
  if (cloud_ring->points.empty () || cloud_ring->size() < min_inlier_points) {
    std::cerr << "Can't find the ring component." << std::endl;
    if(cloud_ring->size() < min_inlier_points) {
        std::cerr << "The number of inliers is too small. Num of points: " << cloud_ring->size() << std::endl;
    }
  }
  else
  {
    ...
    chull.setInputCloud (cloud_ring);
    chull.setAlpha (0.1);
    chull.reconstruct (*hull);

    if(hull->size() < min_hull_size) {
      std::cerr << "Can't find the ring component - no hole big enough. Hole size: " << hull->size() << std::endl;
    } 
    else {*/

      std::cerr << "PointCloud representing the ring component: " << cloud_ring->points.size () << " data points." << std::endl;
      // std::cerr << "Hole is " << hull->size() << " big." << std::endl;
            
            pcl::compute3DCentroid (*cloud_ring, centroid);
            std::cerr << "centroid of the ring component: " << centroid[0] << " " <<  centroid[1] << " " <<   centroid[2] << " " <<   centroid[3] << std::endl;
            ...
            catch (tf2::TransformException &ex)
      {
          ...
          pcl::PCLPointCloud2 outcloud_ring;
            pcl::toPCLPointCloud2 (*cloud_ring, outcloud_ring);
            puby.publish (outcloud_ring);
    //}
  }
...
{
  // Initialize ROS
  ros::init (argc, argv, "ring_segment");
  ros::NodeHandle nh;
  ...
  pubx = nh.advertise<pcl::PCLPointCloud2> ("planes_ring", 1);
