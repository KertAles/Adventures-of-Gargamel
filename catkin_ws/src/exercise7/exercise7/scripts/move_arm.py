...
class Arm_Mover():
    def __init__(self):
        ...
        self.extend.points = [JointTrajectoryPoint(positions=[0.0,0.3,1.0,0],
                                                    time_from_start = rospy.Duration(1))]

        self.touch_cylinder = JointTrajectory()
        self.touch_cylinder.joint_names = ["arm_shoulder_pan_joint", "arm_shoulder_lift_joint", "arm_elbow_flex_joint", "arm_wrist_flex_joint"]
        self.touch_cylinder.points = [JointTrajectoryPoint(positions=[0.2,1.5,0.24,0.0],
                                                    time_from_start = rospy.Duration(1))]

        self.touch_ring = JointTrajectory()
        self.touch_ring.joint_names = ["arm_shoulder_pan_joint", "arm_shoulder_lift_joint", "arm_elbow_flex_joint", "arm_wrist_flex_joint"]
        self.touch_ring.points = [JointTrajectoryPoint(positions=[0.2,0.4,1.2,0],
                                                    time_from_start = rospy.Duration(1))]
    ...
    def update_position(self):
        ...
        if self.send_command:
            if self.user_command == 'retract':
                ...
                print('Extended arm!')
            elif self.user_command == 'touch_cylinder':
                self.arm_movement_pub.publish(self.touch_cylinder)
            elif self.user_command == 'touch_ring':
                self.arm_movement_pub.publish(self.touch_ring)
            else:
