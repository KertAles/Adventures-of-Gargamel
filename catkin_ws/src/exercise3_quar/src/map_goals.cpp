...
void mapCallback(const nav_msgs::OccupancyGridConstPtr& msg_map) {
    ...
    ROS_INFO("map_transform: (x: %d, y: %d), reachable", msg_map->info.origin.position.x, msg_map->info.origin.position.y);
...
void mouseCallback(int event, int x, int y, int, void* data) {
    ...
    ROS_WARN("Able to move to (x: %d, y: %d), reachable", x, y);
    ...
    pt.z = 0.0;
    ROS_INFO("mouse points (x: %f, y: %f), reachable", pt.x, pt.y);
